...
$page_title = 'User Register - Food Order System';
$extra_stylesheets = [
    '../css/checkout.css',
...
<body>
    ...
    (function() {
        ...
        function loadProvinces() {
            fetch(SITEURL + 'api/ghn-address.php?action=province').then(function(r) {
                    return r.json();
                })
                .then(function(res) {
        ...
        function loadDistricts(provinceId) {
            ...
            fetch(SITEURL + 'api/ghn-address.php?action=district&province_id=' + encodeURIComponent(provinceId))
                .then(function(r) {
                    return r.json();
                })
                .then(function(res) {
        ...
        function loadWards(districtId) {
            ...
            fetch(SITEURL + 'api/ghn-address.php?action=ward&district_id=' + encodeURIComponent(districtId)).then(
                    function(r) {
                        return r.json();
                    })
                .then(function(res) {
                    ...
                    list.forEach(function(w) {
                        var code = (w.WardCode != null ? String(w.WardCode) : (w.ward_code ? String(w
                            .ward_code) : '')).trim();
        ...
        selProvince.addEventListener('change', function() {
            loadDistricts(selProvince.value);
        });
        selDistrict.addEventListener('change', function() {
            loadWards(selDistrict.value);
        });
        loadProvinces();
        ...
        foreach ($sessionMessages as $key) {
            if (isset($_SESSION[$key]) && !empty($_SESSION[$key])) {
                ...
                if (!empty($message)) {
                    ...
                    if (
                        strpos(strtolower($_SESSION[$key]), 'success') !== false ||
                        ...
                        strpos(strtolower($_SESSION[$key]), 'error') !== false ||
